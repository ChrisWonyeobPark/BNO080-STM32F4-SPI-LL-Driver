...
/*
 * This library source code has been modified for STM32F4.
 *
 * Development environment specifics:
 * STM32CubeIDE 1.0.0
 * STM32CubeF4 FW V1.24.1
 * STM32F4 LL Driver and HAL Driver
 *
 * Modified by ChrisP(Wonyeob Park) @ M-HIVE Embedded Academy, June, 2019
 * Rev. 1.0
 *
 * https://github.com/ChrisWonyeobPark/BNO080-STM32F4-LL-Driver/
 */
...
char BNO080_SPI_Write(unsigned char addr, unsigned char reg_addr, unsigned char data);
char BNO080_SPI_Writes(unsigned char addr, unsigned char reg_addr, unsigned char len, unsigned char* data);
unsigned char BNO080_SPI_Read(unsigned char addr, unsigned char reg_addr);
char BNO080_SPI_Reads(unsigned char addr, unsigned char reg_addr, unsigned char len, unsigned char* data);
void EXTI9_5_IRQHandler(void);
int BNO080_isNewData(void);
int waitForSPI(void);
float BNO080_qToFloat(int16_t fixedPointValue, uint8_t qPoint);
void BNO080_enableRotationVector(uint32_t microsBetweenReports);
