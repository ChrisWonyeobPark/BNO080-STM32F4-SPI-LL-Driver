...
int BNO080_Initialization(void)
{
	...
	waitForSPI(); //Wait until INT pin goes low.
...
{
	return (BNO080_INT_PORT->IDR & BNO080_INT_PIN) ? 0 : 1; //There is a new data when INT pin is low.
}
...
{
	BNO080_setFeatureCommand(SENSOR_REPORTID_ROTATION_VECTOR, microsBetweenReports, 0);
}
...
int BNO080_receivePacket(void)
{
	...
	//Old way: if (waitForSPI() == 0) return (0); //Something went wrong
